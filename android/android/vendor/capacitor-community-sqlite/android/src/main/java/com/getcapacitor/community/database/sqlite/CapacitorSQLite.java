...
import com.getcapacitor.community.database.sqlite.SQLite.KeyProtectionState;
import com.getcapacitor.community.database.sqlite.SQLite.KeyProtectionStore;
...
public class CapacitorSQLite {
    ...
    private final KeyProtectionStore keyProtectionStore;
    private boolean keyAccessPrepared = false;

    public CapacitorSQLite(Context context, SqliteConfig config) throws Exception {
        this.context = context;
        this.isEncryption = config.getIsEncryption();
        this.keyProtectionStore = new KeyProtectionStore(context);
    }

    private void activatePreferences(SharedPreferences preferences) {
        this.sharedPreferences = preferences;
        this.uSecret = new UtilsSecret(this.context, preferences);
        this.keyAccessPrepared = true;
    }

    public boolean isKeyProtectionEnabled() throws Exception {
        return keyProtectionStore.isEnabledOrEnabling();
    }

    public boolean prepareKeyAccessAfterAuthentication() throws Exception {
        activatePreferences(keyProtectionStore.prepareAfterAuthentication());
        return keyProtectionStore.mode() == KeyProtectionState.Mode.ENABLED;
    }

    public void prepareKeyAccessWithoutAuthentication() throws Exception {
        activatePreferences(keyProtectionStore.prepareDisabled());
    }

    public boolean setKeyProtectionAfterAuthentication(boolean enabled) throws Exception {
        KeyProtectionState.Mode current = keyProtectionStore.mode();
        if (enabled) {
            if (current == KeyProtectionState.Mode.DISABLED) {
                activatePreferences(keyProtectionStore.enableAfterAuthentication());
            } else {
                activatePreferences(keyProtectionStore.prepareAfterAuthentication());
            }
        } else {
            if (current == KeyProtectionState.Mode.ENABLED) {
                activatePreferences(keyProtectionStore.disableAfterAuthentication());
            } else if (current == KeyProtectionState.Mode.ENABLING) {
                SharedPreferences recovered = keyProtectionStore.prepareAfterAuthentication();
                activatePreferences(recovered);
                if (keyProtectionStore.mode() == KeyProtectionState.Mode.ENABLED) {
                    activatePreferences(keyProtectionStore.disableAfterAuthentication());
                }
            } else {
                activatePreferences(keyProtectionStore.prepareDisabled());
    ...
    private void requireKeyAccessPrepared() throws Exception {
        if (isEncryption && !keyAccessPrepared) {
            throw new Exception("Database key access has not been prepared");
        }
    }
    ...
    /**
     * Generate and store the initial SQLCipher passphrase entirely natively.
     *
    ...
    public void ensureEncryptionSecret() throws Exception {
        if (isEncryption) {
            ...
            try {
                closeAllConnections();
                uSecret.ensureEncryptionSecret();
            } catch (Exception e) {
                throw new Exception(e.getMessage());
            }
        } else {
    ...
    public void createConnection(
        ...
        if (encrypted) requireKeyAccessPrepared();
    ...
    /**
     * Deterministic crash-recovery for an interrupted plaintext → SQLCipher
     * conversion of this database (see {@link EncryptionFileSwap} and
     * ADR-0005). Runs BEFORE the public {@link #isDatabase(String)} and
     * {@link #isDatabaseEncrypted(String)} probes return so startup
     * reconciles {@code <db>.encrypting}/{@code <db>.plain.bak} artifacts of
     * a previously killed conversion before any mode decision or open.
     *
     * <p>Idempotent; throws (fail closed) on ambiguous states, never deleting
     * anything it cannot positively verify. Fast no-op when no deterministic
     * conversion artifacts exist.
     */
    private void recoverDatabase(String dbName) throws Exception {
        requireKeyAccessPrepared();
        dbName = getDatabaseName(dbName);
        File file = context.getDatabasePath(dbName + "SQLite.db");
        EncryptionFileSwap swap = new EncryptionFileSwap(file);
        if (!swap.hasArtifacts()) {
            return;
        }
        String storedSecret = "";
        try {
            storedSecret = UtilsSecret.getPassphrase();
        } catch (Exception e) {
            storedSecret = "";
        }
        if (storedSecret == null || storedSecret.isEmpty()) {
            storedSecret = globVar.secret == null ? "" : globVar.secret;
        }
        EncryptionFileSwap.Verifier verifier = uCipher.verifierFor(storedSecret);
        try {
            swap.recover(verifier);
        } catch (IOException e) {
            throw new Exception(
                "plaintext-conversion recovery failed for database " + dbName + ": " + e.getMessage(),
                e
            );
        }
    }
     ...
     * @param dbName database name
     * @return Boolean
     * @throws Exception if crash recovery of an interrupted conversion fails closed
     */
    public Boolean isDatabase(String dbName) throws Exception {
        dbName = getDatabaseName(dbName);
        // Deterministic crash recovery must run before this probe returns.
    ...
    public Boolean isDatabaseEncrypted(String dbName) throws Exception {
        dbName = getDatabaseName(dbName);
        // Deterministic crash recovery runs defensively here too, in case the
        // probe is reached without a preceding successful isDatabase call.
        recoverDatabase(dbName);
        File file = context.getDatabasePath(dbName + "SQLite.db");
