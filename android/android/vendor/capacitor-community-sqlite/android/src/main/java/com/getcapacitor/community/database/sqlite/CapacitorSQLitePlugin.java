...
import com.getcapacitor.community.database.sqlite.SQLite.DeviceAuthenticator;
...
public class CapacitorSQLitePlugin extends Plugin {
    ...
    private PluginCall activeSecurityCall;
    private SecurityAction activeSecurityAction;
    ...
    public void load() {
        ...
        try {
            ...
            deviceAuthenticator = new DeviceAuthenticator(
                context,
            ...
            implementation = null;
            loadMessage = "CapacitorSQLitePlugin unavailable";
    ...
    /** Prepare the authoritative native passphrase store before any SQLite probe. */
    @PluginMethod
    public void prepareKeyAccess(PluginCall call) {
        if (!hasSecurityImplementation(call)) return;
        try {
            if (!implementation.isKeyProtectionEnabled()) {
                implementation.prepareKeyAccessWithoutAuthentication();
                call.resolve(new JSObject().put("enabled", false).put("authenticated", false));
                return;
            }
            beginAuthentication(call, SecurityAction.PREPARE);
        } catch (Exception error) {
            rejectSecurity(call, "key_protection_state");
        }
    }

    /** Read non-secret key-protection status. */
    @PluginMethod
    public void getKeyProtectionStatus(PluginCall call) {
        if (!hasSecurityImplementation(call)) return;
        try {
            call.resolve(
                new JSObject()
                    .put("enabled", implementation.isKeyProtectionEnabled())
                    .put("available", deviceAuthenticator.isAvailable())
            );
        } catch (Exception error) {
            rejectSecurity(call, "key_protection_state");
        }
    ...
    /** Enable or disable authentication-bound passphrase storage. */
    @PluginMethod
    public void setKeyProtection(PluginCall call) {
        if (!hasSecurityImplementation(call)) return;
        Boolean enabled = call.getBoolean("enabled");
        if (enabled == null) {
            rejectSecurity(call, "key_protection_invalid");
            return;
        }
        try {
            boolean current = implementation.isKeyProtectionEnabled();
            if (current == enabled) {
                call.resolve(new JSObject().put("status", "changed").put("enabled", current));
                return;
            }
            beginAuthentication(call, enabled ? SecurityAction.SET_ENABLED : SecurityAction.SET_DISABLED);
        } catch (Exception error) {
            rejectSecurity(call, "key_protection_state");
        }
    }

    /** Fresh system authentication for an already-open in-process app session. */
    @PluginMethod
    public void authenticateSession(PluginCall call) {
        if (!hasSecurityImplementation(call)) return;
        try {
            if (!implementation.isKeyProtectionEnabled()) {
                rejectSecurity(call, "key_protection_disabled");
                return;
            }
            beginAuthentication(call, SecurityAction.SESSION);
        } catch (Exception error) {
            rejectSecurity(call, "key_protection_state");
        }
    }

    private boolean hasSecurityImplementation(PluginCall call) {
        if (implementation != null && deviceAuthenticator != null) return true;
        rejectSecurity(call, "key_protection_unavailable");
        return false;
    }

    private synchronized boolean claimSecurityCall(PluginCall call, SecurityAction action) {
        if (activeSecurityCall != null) {
            rejectSecurity(call, "key_protection_busy");
            return false;
        }
        activeSecurityCall = call;
        activeSecurityAction = action;
        return true;
    }

    private void beginAuthentication(PluginCall call, SecurityAction action) {
        if (!claimSecurityCall(call, action)) return;
        if (!deviceAuthenticator.isAvailable()) {
            rejectActiveSecurity("key_protection_unavailable");
            return;
        }
        try {
            if (deviceAuthenticator.requiresCredentialActivity()) {
                Intent intent = deviceAuthenticator.createCredentialIntent();
                startActivityForResult(call, intent, "credentialAuthenticationResult");
                return;
            }
            ...
            deviceAuthenticator.authenticate(
                ...
                new DeviceAuthenticator.Listener() {
                    @Override
                    public void onAuthenticated() {
                        completeAuthenticatedSecurity();
                    }

                    @Override
                    public void onCancelled() {
                        completeCancelledSecurity();
                    }

                    @Override
                    public void onError() {
                        rejectActiveSecurity("key_protection_failed");
                    }
                }
            );
        } catch (Exception error) {
    ...
    @ActivityCallback
    private void credentialAuthenticationResult(PluginCall call, ActivityResult result) {
        if (call != activeSecurityCall) {
            rejectSecurity(call, "key_protection_state");
            return;
        }
    ...
    private void completeAuthenticatedSecurity() {
        PluginCall call;
        SecurityAction action;
        synchronized (this) {
            call = activeSecurityCall;
            action = activeSecurityAction;
        }
        if (call == null || action == null) return;
        try {
            switch (action) {
                case PREPARE:
                    boolean enabled = implementation.prepareKeyAccessAfterAuthentication();
                    call.resolve(new JSObject().put("enabled", enabled).put("authenticated", enabled));
                    break;
                case SET_ENABLED:
                    call.resolve(
                        new JSObject()
                            .put("status", "changed")
                            .put("enabled", implementation.setKeyProtectionAfterAuthentication(true))
                    );
                    break;
                case SET_DISABLED:
                    call.resolve(
                        new JSObject()
                            .put("status", "changed")
                            .put("enabled", implementation.setKeyProtectionAfterAuthentication(false))
                    );
                    break;
                case SESSION:
                    call.resolve(new JSObject().put("status", "authenticated"));
                    break;
            }
        } catch (Exception error) {
            rejectSecurity(call, "key_protection_state");
    ...
    private void completeCancelledSecurity() {
        PluginCall call;
        SecurityAction action;
        synchronized (this) {
            call = activeSecurityCall;
        ...
        if (call == null || action == null) return;
        try {
            if (action == SecurityAction.PREPARE) {
                rejectSecurity(call, "key_protection_cancelled");
            } else if (action == SecurityAction.SESSION) {
                call.resolve(new JSObject().put("status", "cancelled"));
            } else {
                boolean enabled = implementation.isKeyProtectionEnabled();
                call.resolve(new JSObject().put("status", "cancelled").put("enabled", enabled));
            }
        } catch (Exception error) {
            rejectSecurity(call, "key_protection_state");
        } finally {
    ...
    private void rejectActiveSecurity(String code) {
        PluginCall call;
        synchronized (this) {
            call = activeSecurityCall;
        }
        try {
            if (call != null) rejectSecurity(call, code);
        } finally {
            clearSecurityCall();
        }
    }

    private synchronized void clearSecurityCall() {
        activeSecurityCall = null;
        activeSecurityAction = null;
    }

    private void rejectSecurity(PluginCall call, String code) {
    ...
    /**
     * Generate the initial SQLCipher secret inside native code. The bridge
     * receives only a fixed success marker, never the generated passphrase.
     *
     * @param call PluginCall
     */
    @PluginMethod
    public void ensureEncryptionSecret(PluginCall call) {
        if (implementation == null) {
            rejectSecurity(call, "ensure_encryption_secret_unavailable");
            return;
        }
        try {
            implementation.ensureEncryptionSecret();
            JSObject result = new JSObject();
            result.put("created", true);
            call.resolve(result);
        } catch (Exception ignored) {
            rejectSecurity(call, "ensure_encryption_secret_failed");
        }
    }
        ...
        for (int i = 0; i < set.length(); i++) {
            ...
            for (int j = 0; j < keys.length(); ++j) {
                ...
                if (!key.equals("statement") && !key.equals("values")) {
                    String msg = "ExecuteSet: Must provide a set as Array of {statement,values}";
                    rHandler.retChanges(call, retRes, msg);
                    return;
                }
            }
